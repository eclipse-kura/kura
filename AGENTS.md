...
- **Copyright**: A copyright header is required for any new or updated file and must match the templates below.
  ...
  - **XML header template**:
    ```
        ...
        Contributors:
         {holder}
    ```
    If editing files, always update the copyright year to use the year provided by the system date; add `// Content with portions generated by generative AI platform` after copyright header, if not already present.
