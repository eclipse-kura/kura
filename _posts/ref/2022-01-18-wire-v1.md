...
The WIRE-V1 cloud request handler and REST APIs allow to perform update, delete and get operations on the current Wire Graph. This request handler provides the equivalent of the `org.eclipse.kura.wire.graph.WireGraphService` Java API.

WIRE-V1 is compatible with the Wire Graph import/export functionality of ESF Web UI, the get and update operations respectively consume and produce a JSON object specified by [ComponentConfigurationList]({{ site.baseurl }}{% link _posts/ref/2021-12-13-conf-v2.md %}#componentconfigurationlist).
