...
function create_users {
    ...
    # add kurad to docker group (for container orchestration), only if docker is installed
    if getent group docker >/dev/null 2>&1; then
        gpasswd -a kurad docker
    fi
...
function delete_users {
    ...
    # remove kurad from docker group, only if docker group is present
    if getent group docker >/dev/null 2>&1; then
        gpasswd -d kurad docker
    fi
