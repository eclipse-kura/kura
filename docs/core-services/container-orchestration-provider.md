...
- **Identity Integration**: Automatic provisioning of temporary credentials for containers to securely access Kura's REST APIs, delivered through a read-only token file on an in-memory filesystem (tmpfs) rather than environment variables
