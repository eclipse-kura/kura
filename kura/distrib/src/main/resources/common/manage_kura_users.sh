...
function find_dbus_config_path {
    local config_name=$1
    local default_path="/etc/dbus-1/system.d/${config_name}"
    local fallback_path="/usr/share/dbus-1/system.d/${config_name}"
...
if ((action.id == \"org.freedesktop.login1.reboot-multiple-sessions\" ||
    ...
    BLUETOOTH_CONF_PATH=$(find_dbus_config_path "bluetooth.conf")
    ...
    WPA_SUPPLICANT_CONF_PATH=$(find_dbus_config_path "wpa_supplicant.conf")
    if [ -f "${WPA_SUPPLICANT_CONF_PATH}" ] && ! grep -q kurad "${WPA_SUPPLICANT_CONF_PATH}" && [ $NN == "NO" ]; then
        cp "${WPA_SUPPLICANT_CONF_PATH}" "${WPA_SUPPLICANT_CONF_PATH}.save"
        awk 'done != 1 && /^<\/busconfig>/ {
            print "    <policy user=\"kurad\">"
        ...
        } 1' "${WPA_SUPPLICANT_CONF_PATH}" >tempfile && mv tempfile "${WPA_SUPPLICANT_CONF_PATH}"
    fi
...
function delete_users {
    ...
    # recover old configs
    BLUETOOTH_CONF_PATH=$(find_dbus_config_path "bluetooth.conf.save")
    if [ -f "${BLUETOOTH_CONF_PATH}" ]; then
        mv "${BLUETOOTH_CONF_PATH}" "${BLUETOOTH_CONF_PATH%.save}"
    fi
    WPA_SUPPLICANT_CONF_PATH=$(find_dbus_config_path "wpa_supplicant.conf.save")
    if [ -f "${WPA_SUPPLICANT_CONF_PATH}" ]; then
        mv "${WPA_SUPPLICANT_CONF_PATH}" "${WPA_SUPPLICANT_CONF_PATH%.save}"
    fi
}
