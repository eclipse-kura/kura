...
public class HTS221 {
	...
	public boolean initDevice() {
		...
		if ((read(WHO_AM_I) & 0x000000FF) == WHO_AM_I_ID) {
			...
			int ctrl_reg = read(CTRL_REG1) & 0x000000FF;
			ctrl_reg |= 0x00000083;
			write(CTRL_REG1, ctrl_reg);
	...
	public static void write(int register, int value) {
		try {
			humidityI2CDevice.beginTransaction();
			humidityI2CDevice.write(register);
			humidityI2CDevice.write(value);
			humidityI2CDevice.endTransaction();
		} catch (IOException e) {
			s_logger.error("Unable to read/write to I2C device", e);
		}
	}
