...
public class LPS25H {
	...
	public boolean initDevice() {
		...
		if ((read(WHO_AM_I) & 0x000000FF) == WHO_AM_I_ID) {
			...
			// Set control register : PD = 1 (active mode);  ODR = 011 (12.5 Hz pressure & temperature output data rate)
			write(CTRL_REG1, 0xB0);
		}
	...
	public float getPressure() {
		...
		return (float) (readPressure() / 4096F);
	}

	public float getTemperature() {
		// get the temperature in °C
		return (float) (42.5F + (readTemperature() / 480F));
	}
	...
	public static void write(int register, int value) {
		try {
			pressureI2CDevice.beginTransaction();
			pressureI2CDevice.write(register);
			pressureI2CDevice.write(value);
			pressureI2CDevice.endTransaction();
		} catch (IOException e) {
			s_logger.error("Unable to read/write to I2C device", e);
		}
	}
