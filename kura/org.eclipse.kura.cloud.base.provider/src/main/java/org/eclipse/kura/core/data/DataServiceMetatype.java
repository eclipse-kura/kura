...
@ObjectClassDefinition(
        ...
        name = "DataService",
        description =
                "DataService provides auto-connect, reconnect on connection drops and storing of outgoing messages.",
...
public @interface DataServiceMetatype {
    ...
    @AttributeDefinition(
            ...
            min = "1",
            description =
                    "Frequency in seconds to retry a connection of the Data Publishers after a disconnect (Minimum value 1).")
    int connect_retry$_$interval() default 60;

    @AttributeDefinition(
            name = "Enable Recovery On Connection Failure",
            description =
                    "Enables the recovery feature on connection failure. If the device is not able to connect to a remote cloud platform, the service will wait for a specified amount of connection retries. If the recovery fails, the device will be rebooted. Being based on the Watchdog service, it needs to be activated as well.")
    ...
    @AttributeDefinition(
            ...
            min = "0",
            description =
                    "Timeout used to try to complete the delivery of stored messages before forcing a disconnect of the Data Publisher.")
    int disconnect_quiesce$_$timeout() default 10;

    @AttributeDefinition(
            name = "Message Store Provider Service PID",
            description =
                    "The Kura service pid of the Message Store instance to be used. The pid of the default instance is org.eclipse.kura.db.H2DbService.")
    ...
    @AttributeDefinition(
            ...
            min = "5",
            description =
                    "Age in seconds of completed messages (either published with QoS = 0 or confirmed with QoS > 0) after which they are deleted (min 5).")
    ...
    @AttributeDefinition(
            ...
            min = "0",
            description =
                    "Timeouts the in-flight messages congestion condition. The service will force a disconnect attempting to reconnect (0 to disable).")
    ...
    @AttributeDefinition(
            ...
            min = "1",
            description =
                    "The average message publish rate in number of messages per unit of time (e.g. 10 messages per MINUTE). This parameter has some limitations described on the data service configuration page in the official product documentation.")
    ...
    @AttributeDefinition(
            ...
            description =
                    "A CRON expression that specifies the instants when the gateway should perform a connection attempt. This parameter is only used if Enable Connection Schedule is set to true. The default expression schedules a connection every day at midnight.")
    String connection_schedule_expression() default "0 0 0 ? * * *";

    @AttributeDefinition(
            name = "Allow priority message to overide connection schedule",
            description =
                    "Allows messages beyond a specified priority to force a connection and be sent regardless of connection schedule.")
    boolean connection_schedule_priority_override_enable() default false;

    @AttributeDefinition(
            name = "Message schedule priority override threshold",
            description =
                    "A message with a priority equal to or less than this threshold will cause the framework to automatically re-connect and send regardless of the connection schedule.")
    ...
    @AttributeDefinition(
            ...
            min = "1",
            description =
                    "Specifies an inactivity timeout in seconds. If the timeout expires, the cloud connection will be automatically closed. This timeout is delayed for the specified amount of seconds every time a new message is published. This parameter is only used if Enable Connection Schedule is set to true.")
