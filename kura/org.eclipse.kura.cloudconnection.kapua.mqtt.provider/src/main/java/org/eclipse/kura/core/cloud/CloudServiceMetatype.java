...
@ObjectClassDefinition(id = "org.eclipse.kura.cloud.CloudService", name = "CloudService", description = "The CloudService allows for setting a user friendly name for the current device. It also provides the option to compress message payloads to reduce network traffic.", icon = @Icon(resource = "CloudService", size = 32))
public @interface CloudServiceMetatype {
    ...
    @AttributeDefinition(name = "Topic Control-Prefix", description = "Topic prefix for system and device management messages.")
    String topic_control$_$prefix() default "$EDC";
    ...
    @AttributeDefinition(name = "Republish Mqtt Birth Cert On Tamper Event", description = "Whether or not to republish the MQTT Birth Certificate on a tamper event. This has effect only if a TamperDetectionService is available in the framework.")
    boolean republish_mqtt_birth_cert_on_tamper_event() default true;

    @AttributeDefinition(name = "Enable Default Subscriptions", description = "Manages the default subscriptions to the gateway management MQTT topics. When disabled, the gateway will not be remotely manageable.")
    boolean enable_default_subscriptions() default true;
