...
public class SparkplugCloudEndpointTracker {
    ...
    public SparkplugCloudEndpointTracker(
            BundleContext bundleContext,
            Consumer<SparkplugCloudEndpoint> serviceAddedConsumer,
            Consumer<SparkplugCloudEndpoint> serviceRemovedConsumer,
            String endpointPid) {
    ...
    public void startEndpointTracker() throws InvalidSyntaxException {
        String filterString = String.format(
                ...
                Constants.OBJECTCLASS, CloudConnectionManager.class.getName(), this.endpointPid);

        final Filter filter = this.bundleContext.createFilter(filterString);
        this.cloudConnectionManagerTracker =
                new ServiceTracker<>(this.bundleContext, filter, new SparkplugCloudEndpointTrackerCustomizer());
    ...
    private class SparkplugCloudEndpointTrackerCustomizer
        ...
        public synchronized CloudConnectionManager addingService(
            ...
            CloudConnectionManager cloudConnectionManager =
                    SparkplugCloudEndpointTracker.this.bundleContext.getService(reference);

            if (cloudConnectionManager instanceof SparkplugCloudEndpoint) {
                SparkplugCloudEndpointTracker.this.serviceAddedConsumer.accept(
                        (SparkplugCloudEndpoint) cloudConnectionManager);
