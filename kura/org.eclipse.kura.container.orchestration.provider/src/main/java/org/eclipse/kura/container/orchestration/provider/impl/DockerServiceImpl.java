...
public class DockerServiceImpl implements ConfigurableComponent, DockerService {
	...
	private String itemDownloadingStatus = "";
	...
	public void createLoggerMessageForContainerPull(PullResponseItem item, ContainerDescriptor containerDescription) {
		...
		if (!itemDownloadingStatus.equals(item.getStatus())) {
			...
			if (item.getId() != null) {
				logger.info("Pulling {}:{} Layer {}, State: {}", containerDescription.getContainerImage(),
						containerDescription.getContainerImageTag(), item.getId(), item.getStatus());
			} else {
				logger.info("Pulling {}:{}, State: {}", containerDescription.getContainerImage(),
						containerDescription.getContainerImageTag(), item.getStatus());
			}
	...
	public void stopContainer(ContainerDescriptor container) {
		...
		if (!container.getContainerId().isEmpty()) {
			...
			if (Boolean.FALSE.equals(container.getIsEsfManaged())) {
				// container is not Framework Managed, and thus should only be stopped and not
				// deleted.
				return;
			}
