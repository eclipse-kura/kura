...
public class XmlJavaMetadataMapper implements XmlJavaDataMapper{
	...
	public Element marshal(Document doc, Object o) throws Exception {
		...
		if(o instanceof Tocd ){
			...
			if(ocdADs != null){
				for(AD ocdAD: ocdADs){
					Element ad= mashallDoc.createElement(OCD_NAMESPACE + ":" + METADATA_AD);
					marshallAD(ocdAD, ad);
					ocd.appendChild(ad);
				}
			}
			...
			if(ocdIcons != null){
				for(Icon ocdIcon: ocdIcons){
					Element icon= mashallDoc.createElement(OCD_NAMESPACE + ":" + METADATA_ICON);
	...
	private void marshallIcon(Icon ocdIcon, Element icon) {
		...
		if(iconResource != null && !iconResource.trim().isEmpty()){
			Attr attrResource= mashallDoc.createAttribute(METADATA_ICON_RESOURCE);
			attrResource.setNodeValue(iconResource);
			icon.setAttributeNode(attrResource);
		}
		if(iconSize != null){
			Attr attrSize= mashallDoc.createAttribute(METADATA_ICON_SIZE);
	...
	private  void marshallAD(AD ocdAD, Element ad) {
		...
		if(adOptions != null){
			for(Option adOption: adOptions){
				Element option= mashallDoc.createElement(OCD_NAMESPACE + ":" + METADATA_AD_OPTION);
	...
	private void marshallOption(Option adOption, Element option) {
		...
		if(!label.trim().isEmpty()){
			Attr attrLabel= mashallDoc.createAttribute(METADATA_AD_OPTION_LABEL);
			attrLabel.setNodeValue(label);
			option.setAttributeNode(attrLabel);
		}
		if(!value.trim().isEmpty()){
			Attr attrValue= mashallDoc.createAttribute(METADATA_AD_OPTION_VALUE);
