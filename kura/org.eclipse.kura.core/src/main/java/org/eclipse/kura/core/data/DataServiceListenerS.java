...
class DataServiceListenerS implements DataServiceListener {
	...
	public DataServiceListenerS(ComponentContext ctx) {
		...
		m_listeners = new CopyOnWriteArrayList<DataServiceListener>();
	}
		
	@Override
	public void onConnectionEstablished() {
		Object[] services = m_ctx.locateServices(DATA_SERVICE_LISTENER_REFERENCE);
		if (services != null) {
		...
		if (!m_listeners.isEmpty()) {
			for (DataServiceListener listener : m_listeners) {
				try {
	...
	public void onMessageArrived(String topic, byte[] payload, int qos,
		...
		if (services != null) {
			for (Object service : services) {
				try {
					((org.eclipse.kura.data.DataServiceListener) service).onMessageArrived(topic, payload, qos, retained);
				} catch (Throwable t) {
					s_logger.warn("Unexpected Throwable", t);
				}
			}
