...
public @interface H2DbServerMetatype {
    ...
    @AttributeDefinition(
            ...
            description =
                    "Specifies the server type, see http://www.h2database.com/javadoc/org/h2/tools/Server.html for more details.")
    String db_server_type() default "TCP";

    @AttributeDefinition(
            name = "db.server.commandline",
            description =
                    "Specifies the parameters for the server, see http://www.h2database.com/javadoc/org/h2/tools/Server.html for more details. The listening port must be manually openend in the Firewall configuration section in order to allow external connections.")
