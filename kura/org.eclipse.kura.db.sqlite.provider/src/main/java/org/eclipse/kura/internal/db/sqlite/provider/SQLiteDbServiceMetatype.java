...
@ObjectClassDefinition(
        id = "org.eclipse.kura.db.SQLiteDbService",
...
public @interface SQLiteDbServiceMetatype {
    ...
    @AttributeDefinition(
            ...
            description =
                    "Allows to specify the format of the Encryption Key parameter value. The possible values are ASCII (an ASCII string), Hex SSE (the key is an hexadecimal string to be used with the SSE extension) or Hex SQLCipher (the key is an hexadecimal string to be used with the SQLCipher extension)")
    ...
    @AttributeDefinition(
            ...
            description =
                    "The database journal mode (see https://www.sqlite.org/pragma.html#pragma_journal_mode for more details). If set to Rollback Journal the DELETE journal mode will be used. This parameter is only relevant for persisted databases.")
    String db_journal_mode() default "WAL";

    @AttributeDefinition(
            name = "Defrag enabled",
            description =
                    "Enables or disables the database defragmentation. Use the Defrag Interval parameter to specify the interval.")
    ...
    @AttributeDefinition(
            ...
            min = "60",
            description =
                    "SqliteDbService instances support running periodic defragmentation using the VACUUM command (https://www.sqlite.org/lang_vacuum.html). This parameter specifies the interval in seconds beetween two consecutive defragmentations. This parameter is only relevant for persisted databases.")
    long db_defrag_interval_seconds() default 900L;

    @AttributeDefinition(
            name = "Checkpoint enabled",
            description =
                    "Enables or disables checkpoints in WAL journal mode. Use the WAL Checkpoint Interval parameter to specify the interval.")
    ...
    @AttributeDefinition(
            ...
            min = "1",
            description =
                    "The SqliteDbService manages connections using a connection pool. This parameter defines the maximum number of connections for the pool. Only 1 connection is available in In Memory mode.")
    int db_connection_pool_max_size() default 10;

    @AttributeDefinition(
            name = "Delete Database Files On Failure",
            description =
                    "If set to true, the database files will be deleted in case of failure in opening a persisted database. This is intended as a last resort measure for keeping the database service operational, especially in the case when it is used as a cloud connection message store.")
    boolean delete_db_files_on_failure() default true;

    @AttributeDefinition(
            name = "Debug Shell Access Enabled",
            description =
                    "Enables or disables the interaction with this database instance using the sqlitedbg OSGi console command")
