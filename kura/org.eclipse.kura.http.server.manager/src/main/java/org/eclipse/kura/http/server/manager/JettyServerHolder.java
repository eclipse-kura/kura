...
public class JettyServerHolder {
    ...
    private DeflaterPool deflaterPool =
            new DeflaterPool(CompressionPool.DEFAULT_CAPACITY, Deflater.BEST_COMPRESSION, true);

    private final HttpServiceOptions options;
    private final Server server;
    private final File workDir;

    public JettyServerHolder(
            final HttpServiceOptions options,
            final Optional<KeystoreService> keystoreService,
        ...
        final BundleContext context =
                FrameworkUtil.getBundle(JettyServerHolder.class).getBundleContext();
        ...
        httpContext.addFilter(
                new BlockHttpMethods(EnumSet.of(HttpMethod.TRACE)), "/*", EnumSet.of(DispatcherType.REQUEST));
    ...
    private ServletContextHandler createServletContextHandler() {
        ...
        final SessionCookieConfig cookieConfig =
                servletContextHandler.getSessionHandler().getSessionCookieConfig();
    ...
    public ServerConnector createHttpConnector(int port) {
        ...
        final ServerConnector newConnector =
                new ServerConnector(this.server, new HttpConnectionFactory(httpConfiguration));
    ...
    private static class BlockHttpMethods implements Filter {
        ...
        public BlockHttpMethods(Set<HttpMethod> methods) {
            this.blockedMethods =
                    methods.stream().map(m -> m.toString().toLowerCase()).collect(Collectors.toSet());
        }
    ...
    private ServerConnector createHttpsConnector(
            final KeystoreService keystoreService, final int port, final boolean enableClientAuth)
            throws KuraException {

        final BaseSslContextFactory sslContextFactory;

        if (enableClientAuth) {
            sslContextFactory = new ClientAuthSslContextFactoryImpl(
                    keystoreService, this.options.isRevocationEnabled(), getRevocationCheckerOptions());
        } else {
