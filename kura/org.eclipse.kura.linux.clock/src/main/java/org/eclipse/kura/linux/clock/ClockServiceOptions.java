...
public @interface ClockServiceOptions {
    ...
    @AttributeDefinition(
            ...
            description =
                    "Source for setting the system clock. Verify the availabiliy of the selected provider before activate it. Using chrony-advanced causes all fields, except Chrony Configuration, to be ignored.")
    ...
    @AttributeDefinition(
            ...
            min = "0",
            description =
                    "The maximum number of retries for the initial synchronization (with interval clock.ntp.retry.interval). If set to 0 the service will retry forever.")
    ...
    @AttributeDefinition(
            ...
            description =
                    "Whether or not to sync the clock and if so, the frequency in seconds.  If less than zero - no update, if equal to zero - sync once at startup, if greater than zero - the frequency in seconds to perform a new clock sync")
    int clock_ntp_refresh$_$interval() default 3600;

    @AttributeDefinition(
            name = "RTC File Name",
            description =
                    "The RTC File Name. It defaults to /dev/rtc0. This option is not used if chrony-advanced option is selected in clock.provider.")
