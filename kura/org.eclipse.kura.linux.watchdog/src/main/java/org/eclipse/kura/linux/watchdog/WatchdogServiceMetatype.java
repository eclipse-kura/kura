...
public @interface WatchdogServiceMetatype {
    ...
    @AttributeDefinition(
            ...
            description =
                    "The WatchdogService monitors CriticalComponents and reboots the system if one of them hangs. Once enabled the WatchdogService starts refreshing the watchdog device, which will reset the system if WatchdogService hangs.")
    ...
    @AttributeDefinition(
            ...
            max = "60000",
            description =
                    "WatchdogService's refresh interval in ms of the Watchdog device. The value can be set between 1 and 60 seconds and should not be set to a value greater or equal to the Watchdog device's timeout value")
