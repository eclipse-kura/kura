...
public class ModbusManager implements ConfigurableComponent, CriticalComponent, CloudClientListener {
    ...
    private boolean doConnectionWork() {
        ...
        try {
            iJustConnected = true;
            this.m_cloudAppClient.controlSubscribe(topic + "/led/#", 0);
            this.m_cloudAppClient.controlSubscribe(topic + "/resetcnt/#", 0);
            this.m_cloudAppClient.controlSubscribe(topic + "/alarm", 0);
    ...
    private Properties getModbusProperties() {
        ...
        if (this.m_properties != null) {
            ...
            if (this.m_properties.get("ipAddress") != null) {
                ipAddress = (String) this.m_properties.get("ipAddress");
            }
    ...
    public void TurnOnLED(int LED, boolean On) throws ModbusProtocolException {
        ...
        try {
            switch (LED) {
            case 1:
                s_logger.debug("Setting LED" + LED + " to " + On);
                break;
            case 2:
                s_logger.debug("Setting LED" + LED + " to " + On);
                break;
            case 3:
                ...
                break;
            case 4:
                s_logger.debug("Setting LED4 red to " + On);
                break;
            case 5:
                s_logger.debug("Setting LED4 green to " + On);
                break;
            case 6:
                s_logger.debug("Setting LED4 blue to " + On);
                break;
            default:
    ...
    public void clearCounter(int counter, boolean On) {
        ...
        try {
            ...
            this.m_protocolDevice.writeSingleCoil(this.slaveAddr, 3072 + counter, On ? TurnON : TurnOFF);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
