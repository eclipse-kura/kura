...
public class UpdateComponentConfigurationRequest implements Validable {
    ...
    public UpdateComponentConfigurationRequest(
            List<ComponentConfigurationDTO> componentConfigurations, boolean takeSnapshot) {
        ...
        for (final ComponentConfigurationDTO config : this.configs) {
            if (config == null) {
                throw DefaultExceptionHandler.buildWebApplicationException(
                        Status.BAD_REQUEST, "component configuration objects cannot be null");
            }
