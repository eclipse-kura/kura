...
public class CertificateAuthenticationProvider implements AuthenticationProvider {
    ...
    public Optional<Principal> authenticate(
            final ContainerRequestContext requestContext, final String auditAuthenticationKind) {
    ...
    public Principal authenticate(final ContainerRequestContext requestContext) throws CertificateAuthException {
        ...
        try {
            ...
            final LdapName ldapName =
                    new LdapName(clientCertificates[0].getSubjectX500Principal().getName());

            final Optional<Rdn> commonNameRdn = ldapName.getRdns().stream()
                    .filter(r -> "cn".equalsIgnoreCase(r.getType()))
                    .findAny();
