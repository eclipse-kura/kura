...
public class SessionRestService {
    ...
    private static final String BAD_USERNAME_OR_PASSWORD_MESSAGE =
            "Authentication failed as username or password not matching";
    private static final String PASSWORD_CHANGE_SAME_PASSWORD_MESSAGE =
            "Password change failed as previous password equals new one";
    ...
    public SessionRestService(
            final RestIdentityHelper identityHelper,
            final RestSessionHelper restSessionHelper,
    ...
    public AuthenticationResponseDTO authenticateWithUsernameAndPassword(
            final UsernamePasswordDTO usernamePassword, @Context final HttpServletRequest request) {
        ...
        try {
            ...
            this.identityHelper.checkPassword(
                    usernamePassword.getUsername(),
                    usernamePassword.getPassword().toCharArray());

            final HttpSession session =
                    this.restSessionHelper.createNewAuthenticatedSession(request, usernamePassword.getUsername());

            final AuthenticationResponseDTO response = buildAuthenticationResponse(
                    usernamePassword.getUsername(), this.loginBannerService.getPostLoginBanner());
            ...
            throw DefaultExceptionHandler.buildWebApplicationException(
                    Status.UNAUTHORIZED, BAD_USERNAME_OR_PASSWORD_MESSAGE);
    ...
    @Produces(MediaType.APPLICATION_JSON)
    public AuthenticationResponseDTO authenticateWithCertificate(
            @Context final HttpServletRequest request, @Context final ContainerRequestContext requestContext) {
        ...
        try {
            ...
            final CertificateAuthenticationProvider certificateAuthProvider =
                    new CertificateAuthenticationProvider(identityHelper);

            final Optional<Principal> principal = certificateAuthProvider.authenticate(
                    requestContext, "Create session via certificate authentication");

            if (principal.isPresent()) {
                this.restSessionHelper.createNewAuthenticatedSession(
                        request, principal.get().getName());
            } else {

                throw DefaultExceptionHandler.buildWebApplicationException(
                        Status.UNAUTHORIZED, "Certificate authentication failed");
            }
    ...
    public XsrfTokenDTO getXSRFToken(
            @Context final HttpServletRequest request, @Context final ContainerRequestContext requestContext) {
    ...
    public void updateUserPassword(
        ...
        try {
            final Optional<String> username = this.restSessionHelper
                    .getCurrentPrincipal(requestContext)
                    .flatMap(c -> Optional.ofNullable(c.getName()));

            if (!username.isPresent()) {
                throw DefaultExceptionHandler.buildWebApplicationException(
                        Status.UNAUTHORIZED, INVALID_SESSION_MESSAGE);
            }

            this.identityHelper.checkPassword(
                    username.get(), passwordUpdate.getCurrentPassword().toCharArray());
            ...
            if (isSamePassword(username.get(), newPassword)) {
                throw DefaultExceptionHandler.buildWebApplicationException(
                        Status.BAD_REQUEST, PASSWORD_CHANGE_SAME_PASSWORD_MESSAGE);
            }
            ...
            if (e.getCode() == KuraErrorCode.SECURITY_EXCEPTION) {
                auditLogger.warn(
                        AUDIT_FORMAT_STRING, AuditContext.currentOrInternal(), BAD_USERNAME_OR_PASSWORD_MESSAGE);
                throw DefaultExceptionHandler.buildWebApplicationException(
                        Status.UNAUTHORIZED, BAD_USERNAME_OR_PASSWORD_MESSAGE);
            }
    ...
    @Produces(MediaType.APPLICATION_JSON)
    public IdentityInfoDTO getCurrentIdentityInfo(
            @Context final ContainerRequestContext requestContext, @Context final HttpServletRequest request) {
        if (!options.isSessionManagementEnabled()) {
            throw new WebApplicationException(Status.NOT_FOUND);
        }
    ...
    public AuthenticationInfoDTO getAuthenticationMethodInfo() {
        ...
        final String preLoginBannerMessage =
                loginBannerService.getPreLoginBanner().orElse(null);
        ...
        final Map<String, Object> httpServiceConfig =
                ConfigurationAdminHelper.loadHttpServiceConfigurationProperties(configAdmin);
    ...
    private AuthenticationResponseDTO buildAuthenticationResponse(
            final String username, final Optional<String> message) {
        final boolean needsPasswordChange;
        try {
            needsPasswordChange = this.identityHelper.isPasswordChangeRequired(username);
        } catch (final Exception e) {
            throw DefaultExceptionHandler.buildWebApplicationException(
                    Status.INTERNAL_SERVER_ERROR, "An internal error occurred");
        }
