...
{	
	...
	private List<ScheduledFuture<?>>    m_handle;
	...
	{
		super();
		m_worker = Executors.newScheduledThreadPool(5);
		m_handle = new ArrayList<ScheduledFuture<?>>();
	}	
	...
	private void doUpdate() 
	{
		...
		for (ScheduledFuture<?> handle : m_handle) {
			if (handle != null) {
				handle.cancel(true);
			}
		}
				
		final boolean enable = (Boolean) m_properties.get(HEAP_ENABLE_PROP_NAME);
		final int threads = (Integer) m_properties.get(HEAP_THREADS_PROP_NAME);
		final int size = 1024 * (Integer) m_properties.get(HEAP_KBYTES_PROP_NAME);
		final int stride = 1024 * (Integer) m_properties.get(HEAP_STRIDE_KBYTES_PROP_NAME);
		final int hang = (Integer) m_properties.get(HEAP_HANG_MS_PROP_NAME);
		final int interval = (Integer) m_properties.get(HEAP_INTERVAL_MS_PROP_NAME);
		final boolean log = (Boolean) m_properties.get(HEAP_LOG_PROP_NAME);
		final int delay = (Integer) m_properties.get(HEAP_DELAY_MS_PROP_NAME);
		
		if (enable) {
			for (int i = 0; i < threads; i++) {
				final int tid = i;
				ScheduledFuture<?> handle = m_worker.scheduleAtFixedRate(new Runnable() {		
					@Override
					public void run() {
