...
public class GwtBSWifiChannelModel extends GwtBSBaseModel {
	...
	public GwtBSWifiChannelModel () {
		
	}
	
	public GwtBSWifiChannelModel (String name, int channel, int frequency, String band) {
		
		set("name", String.valueOf(name));
		set("channel", String.valueOf(channel));
		set("frequency", String.valueOf(frequency));
		set("band", String.valueOf(band));
	}
	...
	public static List<GwtBSWifiChannelModel> getChannels() {
		
		List<GwtBSWifiChannelModel> alCannels = new ArrayList<GwtBSWifiChannelModel> ();
		for (int i = FIRST_2400MHZ_CHANNEL; i <= LAST_2400MHZ_CHANNEL; i++) {
			alCannels.add(new GwtBSWifiChannelModel(formChannelName(i), i, getCannelFrequencyMHz(i), BAND2400MHZ));
		}
		return alCannels;
	}
