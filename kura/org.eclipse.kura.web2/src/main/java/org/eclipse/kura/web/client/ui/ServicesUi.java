...
public class ServicesUi extends Composite {
    ...
    private void renderTextField(final GwtConfigParameter param, boolean isFirstInstance, FormGroup formGroup) {
        ...
        // TODO: Probably this formatting step has no sense. But it seems that,
        // if not in debug, all the browsers are able to display the double
        // value as expected
        switch (param.getType()) {
            case LONG:
    ...
    private boolean validate(GwtConfigParameter param, TextBox box, FormGroup group) {  // TODO:
        // validation
        // should
        // be
        // done
        ...
        if (box.getText().trim() != null && !"".equals(box.getText().trim())) {
            if (param.getType().equals(GwtConfigParameterType.CHAR)) {
                ...
                if (param.getMin() != null) {
                    if (Character.valueOf(param.getMin().charAt(0)).charValue() > Character.valueOf(box.getText().trim().charAt(0)).charValue()) {  // TODO:
                        // why
                        // this
                        // character
                        // boxing?
