...
public class GwtStatusServiceImpl extends OsgiRemoteServiceServlet implements GwtStatusService {
    ...
    private List<GwtGroupedNVPair> getCloudStatus() throws GwtKuraException {
        ...
        try {
            // FIXME: should be provided in stable order
    ...
    private void fillFromCloudService(List<GwtGroupedNVPair> pairs, ServiceReference<CloudService> cloudServiceReference, String cloudServicePid) throws GwtKuraException {
        ...
        try {
            pairs.add(new GwtGroupedNVPair("cloudStatus", "Service Status", cloudService.isConnected() ? "CONNECTED" : "DISCONNECTED"));

            final String dataServiceRef = (String) cloudServiceReference.getProperty(DATA_SERVICE_REFERENCE_NAME + ComponentConstants.REFERENCE_TARGET_SUFFIX);
            ...
            try {
                if (dataService != null) {
                    ...
                    pairs.add(new GwtGroupedNVPair("cloudStatus", "Auto-connect",
                            dataService.isAutoConnectEnabled() ? "ON (Retry Interval is " + Integer.toString(dataService.getRetryInterval()) + "s)" : "OFF"));
                }
