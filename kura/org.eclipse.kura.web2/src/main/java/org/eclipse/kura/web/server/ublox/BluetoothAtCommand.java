...
public class BluetoothAtCommand {
	...
	private static final Logger logger = Logger.getLogger(BluetoothAtCommand.class);
	...
	public static short rssi = 0;
	public static long Atscan_timeout = 0; // time out of the discovery
     ...
     *
     * @return conn_handle (type integer),attr_handle (type integer),propreties (byte array), value handle (type integer), uuid (type hex string)
     */
	  ...
	  *             
	  *
	  * @return conn_handle (type integer),char_handle (type integer),descriptor_handle (type integer), value handle (type int), uuid (type hex string)
	  */
	 ...
	 /**
	     ...
	     *            1 reliable
	     * @param flag
	     *           optional flag that is used when sending several packets or when the data is cancelled. if you send several
	     *           packets, all but the last packet should set the flag to more data. the last data packet should set the flag to final
	     *           0 =>final data(default), 1 =>more data, 2=> cancel data writing
	     * @param offset
	     * @return 
	     */
	 public void writeLongCharacteristicValue(int value_handle, String hex_data, String reliable, int flag,String offset) {
	  
	 try {    		
	       serial.sendText("AT+UBTGWL="+conn_handle+","+value_handle+","+hex_data+","+reliable+","+flag+","+offset);
	    		} catch (Exception e) {
	    			e.printStackTrace();
	 }	
