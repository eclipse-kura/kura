...
public class WatchdogServiceImpl implements WatchdogService, ConfigurableComponent {
	...
	private static ArrayList<CriticalComponentImpl>	s_criticalComponentList;
	...
	protected void activate(ComponentContext componentContext, Map<String,Object> properties) {
		...
		s_criticalComponentList = new ArrayList<CriticalComponentImpl>();
		m_enabled=false;
    ...
    public void registerCriticalComponent(CriticalComponent criticalComponent) {
        ...
        ", list contains " + s_criticalComponentList.size() + " critical services");
    }
    ...
    public void unregisterCriticalComponent(CriticalComponent criticalComponent) {
        synchronized(s_criticalComponentList) {
            for(int i=0; i<s_criticalComponentList.size(); i++) {
                if(criticalComponent.getCriticalComponentName().compareTo(((CriticalComponentImpl)s_criticalComponentList.get(i)).getName())==0) {
                    s_criticalComponentList.remove(i);
	...
	private void doWatchdogLoop() {
		...
		// Critical Services
		synchronized(s_criticalComponentList) {
			if(s_criticalComponentList.size() > 0) {
				for(CriticalComponentImpl csi:s_criticalComponentList){
					if(csi.isTimedOut()) {
						failure=true;
