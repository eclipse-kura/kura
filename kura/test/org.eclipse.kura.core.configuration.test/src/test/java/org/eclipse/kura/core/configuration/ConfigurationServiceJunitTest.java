...
public class ConfigurationServiceJunitTest {
    ...
    @Test
    public void testGetTempSnapshotFileSuccess() throws Throwable {
        // Test successful temporary file creation
        ConfigurationServiceImpl cs = new ConfigurationServiceImpl();
        
        final String dir = "tempSnapDir";
        File parentDir = new File(dir);
        parentDir.mkdirs();
        parentDir.deleteOnExit();
        
        File originalFile = new File(parentDir, "snapshot_123.xml");
        
        File tempFile = (File) TestUtil.invokePrivate(cs, "getTempSnapshotFile", originalFile);
        
        assertNotNull("Temp file should be created", tempFile);
        assertTrue("Temp file should exist", tempFile.exists());
        assertTrue("Temp file name should start with original name", 
                   tempFile.getName().startsWith("snapshot_123.xml"));
        assertEquals("Temp file should be in same directory", 
                     originalFile.getParent(), tempFile.getParent());
    ...
    @Test
    public void testGetTempSnapshotFileIOException() throws Throwable {
        // Test IOException handling in temp file creation
        ConfigurationServiceImpl cs = new ConfigurationServiceImpl();
        
        // Use a non-existent parent directory to trigger IOException
        File originalFile = new File("/non/existent/path/snapshot_123.xml");
        
        try {
            TestUtil.invokePrivate(cs, "getTempSnapshotFile", originalFile);
    ...
    @Test
    public void testMoveTempFileSuccess() throws Throwable {
        // Test successful file move with POSIX permissions
        ConfigurationServiceImpl cs = new ConfigurationServiceImpl();
        
        final String dir = "moveTestDir";
        File parentDir = new File(dir);
        parentDir.mkdirs();
        parentDir.deleteOnExit();
        
        File tempFile = new File(parentDir, "temp_snapshot.xml");
        tempFile.createNewFile();
        try (FileWriter fw = new FileWriter(tempFile)) {
            fw.write("test content");
        }
        
        File targetFile = new File(parentDir, "snapshot_123.xml");
        
        // Test the move operation
        TestUtil.invokePrivate(cs, "moveTempFile", targetFile, tempFile);
        
        assertFalse("Temp file should be moved", tempFile.exists());
        ...
        try (FileReader fr = new FileReader(targetFile)) {
            ...
            assertEquals("Content should be preserved", "test content", content);
        }
    ...
    @Test
    public void testMoveTempFileUnsupportedOperation() throws Throwable {
        // Test handling of UnsupportedOperationException for POSIX permissions
        ConfigurationServiceImpl cs = new ConfigurationServiceImpl() {
            // This test may not trigger UnsupportedOperationException on all systems,
            // but we'll test the code path exists
        };
        
        final String dir = "moveTestDirUnsupported";
        File parentDir = new File(dir);
        parentDir.mkdirs();
        parentDir.deleteOnExit();
        ...
        // Move should still succeed even if POSIX permissions fail
        TestUtil.invokePrivate(cs, "moveTempFile", targetFile, tempFile);
    ...
    @Test
    public void testMoveTempFileIOException() throws Throwable {
        // Test IOException handling in file move
        ConfigurationServiceImpl cs = new ConfigurationServiceImpl();
        
        File tempFile = new File("/tmp/temp_snapshot.xml");
        tempFile.createNewFile();
        tempFile.deleteOnExit();
        
        // Try to move to a non-existent directory
        File targetFile = new File("/non/existent/path/snapshot_123.xml");
    ...
    @Test
    public void testWriteSnapshotWithTempFileCleanup() throws Throwable {
        // Test that temp files are cleaned up even when marshalling fails
        long sid = 456L;

        final String dir = "snapshotDirTempCleanup";
        File d1 = new File(dir);
        d1.mkdirs();
        d1.deleteOnExit();
        ...
        ConfigurationServiceImpl cs = new ConfigurationServiceImpl() {
            ...
            @Override
            protected String marshal(Object object) {
                return null; // Force marshalling to fail and return null
            }
        };
        ...
        try {
            TestUtil.invokePrivate(cs, "writeSnapshot", sid, cfg);
            fail("Exception expected due to null marshalling result");
        } catch (KuraException e) {
            assertEquals("Error code should be INVALID_PARAMETER", KuraErrorCode.INVALID_PARAMETER, e.getCode());
        }

        // Verify no temp files are left behind
        File[] tempFiles = d1.listFiles((dir1, name) -> name.contains("snapshot_") && name.contains(".xml"));
        assertEquals("No temp files should remain", 0, tempFiles != null ? tempFiles.length : 0);

        d1.delete();
    }

    @Test
    public void testWriteSnapshotTempFileCreationFailure() throws Throwable {
        // Test handling when temp file creation fails
        long sid = 789L;
        ...
        ConfigurationServiceImpl cs = new ConfigurationServiceImpl() {
            ...
            @Override
            String getSnapshotsDirectory() {
                return "/non/existent/path"; // This will cause temp file creation to fail
            }
        ...
        CryptoService cryptoServiceMock = mock(CryptoService.class);
        cs.setCryptoService(cryptoServiceMock);
        when(cryptoServiceMock.encryptAes(any(char[].class))).thenReturn("encrypted".toCharArray());

        try {
            TestUtil.invokePrivate(cs, "writeSnapshot", sid, cfg);
            fail("Expected KuraIOException due to temp file creation failure");
        } catch (KuraIOException e) {
            // Expected exception
            assertTrue("Exception should be of type KuraIOException", e instanceof KuraIOException);
        }
    ...
    @Test
    public void testWriteSnapshotAtomicMoveSuccess() throws Throwable {
        // Test successful atomic move of temp file to final location
        long sid = 101L;

        XmlComponentConfigurations cfg = prepareSnapshot();

        final String dir = "snapshotDirAtomicMove";
        File d1 = new File(dir);
        d1.mkdirs();
        d1.deleteOnExit();
        ...
        String encContent = "encrypted_snapshot_content";
        when(cryptoServiceMock.encryptAes(any(char[].class))).thenReturn(encContent.toCharArray());

        TestUtil.invokePrivate(cs, "writeSnapshot", sid, cfg);

        File finalFile = new File(d1, "snapshot_" + sid + ".xml");
        assertTrue("Final snapshot file should exist", finalFile.exists());

        // Verify content
        try (FileReader fr = new FileReader(finalFile)) {
            char[] buffer = new char[encContent.length()];
            int len = fr.read(buffer);
            String actualContent = new String(buffer, 0, len);
            assertEquals("Content should match encrypted content", encContent, actualContent);
        }

        // Verify no temp files remain
        File[] tempFiles = d1.listFiles((dir1, name) -> 
            name.startsWith("snapshot_" + sid + ".xml") && !name.equals("snapshot_" + sid + ".xml"));
        assertEquals("No temp files should remain", 0, tempFiles != null ? tempFiles.length : 0);
    ...
    @Test
    public void testMoveTempFileKuraIOException() throws Throwable {
    ...
    @Test
    public void testWriteSnapshotTempFileCleanupFailure() throws Throwable {
        // Test that the method continues even if temp file cleanup fails
        long sid = 303L;

        XmlComponentConfigurations cfg = prepareSnapshot();

        final String dir = "snapshotDirCleanupFailure";
        File d1 = new File(dir);
        d1.mkdirs();
        d1.deleteOnExit();
        ...
        ConfigurationServiceImpl cs = new ConfigurationServiceImpl() {
            ...
            // Override to capture temp file reference for testing cleanup failure
            private File getTempSnapshotFile(File fSnapshot) throws KuraIOException {
                try {
                    File tempFile = File.createTempFile(fSnapshot.getName(), null, new File(fSnapshot.getParent()));
                    tempFileRef[0] = tempFile;
                    // Make the temp file read-only to simulate cleanup failure
        ...
        // This should succeed despite cleanup failure
        TestUtil.invokePrivate(cs, "writeSnapshot", sid, cfg);
        ...
        if (tempFileRef[0] != null && tempFileRef[0].exists()) {
            tempFileRef[0].setWritable(true);
    ...
    @Test
    public void testWriteSnapshotEmptyContent() throws Throwable {
        // Test writing snapshot with empty marshalled content
        long sid = 404L;

        XmlComponentConfigurations cfg = prepareSnapshot();

        final String dir = "snapshotDirEmpty";
        File d1 = new File(dir);
        d1.mkdirs();
        d1.deleteOnExit();
        ...
        try {
            TestUtil.invokePrivate(cs, "writeSnapshot", sid, cfg);
            fail("Exception expected for empty marshalled content");
        } catch (KuraException e) {
            assertEquals("Error code should be INVALID_PARAMETER", KuraErrorCode.INVALID_PARAMETER, e.getCode());
        }
    ...
    @Test
    public void testWriteSnapshotWhitespaceOnlyContent() throws Throwable {
        // Test writing snapshot with whitespace-only marshalled content
        long sid = 505L;

        XmlComponentConfigurations cfg = prepareSnapshot();

        final String dir = "snapshotDirWhitespace";
        File d1 = new File(dir);
        d1.mkdirs();
        d1.deleteOnExit();
        ...
        try {
            TestUtil.invokePrivate(cs, "writeSnapshot", sid, cfg);
            fail("Exception expected for whitespace-only marshalled content");
        } catch (KuraException e) {
            assertEquals("Error code should be INVALID_PARAMETER", KuraErrorCode.INVALID_PARAMETER, e.getCode());
        }
    ...
    @Test
    public void testWriteSnapshotFileAlreadyExists() throws Throwable {
        // Test writing snapshot when target file already exists (should overwrite)
        long sid = 606L;

        XmlComponentConfigurations cfg = prepareSnapshot();

        final String dir = "snapshotDirOverwrite";
        File d1 = new File(dir);
        d1.mkdirs();
        d1.deleteOnExit();

        // Create existing file with different content
        File existingFile = new File(d1, "snapshot_" + sid + ".xml");
        ...
        CryptoService cryptoServiceMock = mock(CryptoService.class);
        cs.setCryptoService(cryptoServiceMock);
        String newContent = "new encrypted content";
        when(cryptoServiceMock.encryptAes(any(char[].class))).thenReturn(newContent.toCharArray());

        TestUtil.invokePrivate(cs, "writeSnapshot", sid, cfg);

        assertTrue("Final snapshot file should exist", existingFile.exists());

        // Verify content was overwritten
        try (FileReader fr = new FileReader(existingFile)) {
            char[] buffer = new char[newContent.length()];
            int len = fr.read(buffer);
            String actualContent = new String(buffer, 0, len);
            assertEquals("Content should be overwritten", newContent, actualContent);
        }
