...
public class RestServiceTest extends AbstractRequestHandlerTest {
    ...
    @Test
    public void shouldCreateTokenPairWithUsernameAndPasswordV2() {
        givenIdentityTokenService();
        givenService(new RequiresAssetsRole());
        givenIdentity("foo", Optional.of("bar"), Collections.emptyList());
        givenNoBasicCredentials();

        whenRequestIsPerformed("http", 8080, new MethodSpec("POST"), "/session/v2/login/password",
                "{\"username\":\"foo\",\"password\":\"bar\"}");

        thenResponseCodeIs(200);
        thenCurrentTokenPairIsAvailable();
    }

    @Test
    public void shouldAllowAccessingResourceThroughJwtBearerAuthentication() {
        givenIdentityTokenService();
        givenService(new RequiresAssetsRole());
    ...
    @Test
    public void shouldRotateRefreshTokenAndRejectReuse() {
        givenIdentityTokenService();
        givenIdentity("foo", Optional.of("bar"), Collections.emptyList());
        givenNoBasicCredentials();
        givenSuccessfulRequest("http", 8080, new MethodSpec("POST"), "/session/v2/login/password",
        ...
        final String initialRefreshToken = this.currentRefreshToken;

        whenRequestIsPerformed("http", 8080, new MethodSpec("POST"), "/session/v2/token/refresh",
                "{\"refreshToken\":\"" + initialRefreshToken + "\"}");

        thenResponseCodeIs(200);
        thenCurrentTokenPairIsAvailable();
        assertNotEquals(initialRefreshToken, this.currentRefreshToken);
    ...
    @Test
    public void shouldRejectJwtBearerAfterLogoutV2() {
        givenIdentityTokenService();
        givenService(new RequiresAssetsRole());
        givenIdentity("foo", Optional.of("bar"), Arrays.asList("rest.assets"));
        givenNoBasicCredentials();
        ...
        givenSuccessfulRequest("http", 8080, new MethodSpec("POST"), "/session/v2/logout", null);
    ...
    private String currentTokenFamilyId;
    private final AtomicLong tokenCounter = new AtomicLong();
    private final Map<String, AccessTokenInfo> accessTokensByValue = new ConcurrentHashMap<>();
    private final Map<String, TokenPairInfo> refreshTokens = new ConcurrentHashMap<>();
    private Class<?> tokenPairClass;
    private Class<?> verifiedAccessTokenClass;
    ...
    public void givenCurrentAccessTokenAsBearerHeader() {
        ((RestTransport) this.transport).setHeader("Authorization", "Bearer " + this.currentAccessToken);
    }
    ...
    private void givenIdentityTokenService() {
        ...
        try {
            ...
            final Class<?> serviceClass = restProviderBundle.loadClass("org.eclipse.kura.identity.IdentityTokenService");
            this.tokenPairClass = restProviderBundle.loadClass("org.eclipse.kura.identity.TokenPair");
            this.verifiedAccessTokenClass = restProviderBundle.loadClass("org.eclipse.kura.identity.VerifiedAccessToken");

            final Object service = Proxy.newProxyInstance(serviceClass.getClassLoader(), new Class[] { serviceClass },
                    new InMemoryIdentityTokenServiceHandler());
            this.registeredServices.add(bundleContext.registerService(serviceClass.getName(), service, null));
            givenRestServiceConfiguration(map( //
                    "auth.password.enabled", true, //
                    "auth.jwt.enabled", true, //
                    "session.management.enabled", true));
            waitUntilIdentityTokenServiceIsBound();
        } catch (final Exception e) {
            throw new IllegalStateException("Cannot register test IdentityTokenService", e);
        }
    }

    private void waitUntilIdentityTokenServiceIsBound() {
        final RestTransport restTransport = (RestTransport) this.transport;
        final MethodSpec method = new MethodSpec("POST");
        final String body = "{\"username\":\"probe\",\"password\":\"probe\"}";
        int lastStatus = -1;

        for (int i = 0; i < 300; i++) {
            final Response probeResponse = restTransport.runRequest("http://localhost:8080/services",
                    "/session/v2/login/password", method, body);
            lastStatus = probeResponse.getStatus();
            ...
            synchronized (this) {
                try {
                    ...
                    fail("Interrupted while waiting IdentityTokenService binding");
                    return;
                }
            }
        }
    ...
    private void thenCurrentTokenPairIsAvailable() {
        updateCurrentTokenPair();
        assertTrue(this.currentAccessToken != null && !this.currentAccessToken.isEmpty());
        assertTrue(this.currentRefreshToken != null && !this.currentRefreshToken.isEmpty());
        assertTrue(this.currentTokenFamilyId != null && !this.currentTokenFamilyId.isEmpty());
    }

    private void updateCurrentTokenPair() {
        final Optional<String> body = expectResponse().getBody();
        if (!body.isPresent()) {
            return;
        }

        final String responseBodyRaw = body.get();
        if (!responseBodyRaw.contains("\"accessToken\"") || !responseBodyRaw.contains("\"refreshToken\"")
                || !responseBodyRaw.contains("\"tokenFamilyId\"")) {
            return;
        }

        final JsonObject responseBody;
        try {
            responseBody = Json.parse(responseBodyRaw).asObject();
        } catch (RuntimeException e) {
            return;
        }
        if (responseBody.get("accessToken") == null || responseBody.get("refreshToken") == null
                || responseBody.get("tokenFamilyId") == null) {
            return;
        }

        this.currentAccessToken = responseBody.get("accessToken").asString();
        this.currentRefreshToken = responseBody.get("refreshToken").asString();
        this.currentTokenFamilyId = responseBody.get("tokenFamilyId").asString();
        assertEquals(3, this.currentAccessToken.split("\\.").length);
    }

    private class InMemoryIdentityTokenServiceHandler implements InvocationHandler {
    ...
    private Object issueTokenPair(final String identityName) {
        return issueTokenPairInternal(identityName, null);
    }

    private Object refreshTokenPair(final String refreshToken) throws KuraException {
        final TokenPairInfo previous = refreshTokens.remove(refreshToken);
        if (previous == null) {
            throw new KuraException(KuraErrorCode.SECURITY_EXCEPTION, "Invalid refresh token");
        }

        final AccessTokenInfo previousAccessToken = accessTokensByValue.get(previous.accessToken);
        if (previousAccessToken == null) {
            throw new KuraException(KuraErrorCode.SECURITY_EXCEPTION, "Invalid refresh token");
        }

        return issueTokenPairInternal(previousAccessToken.identityName, previous.tokenFamilyId);
    }

    private Optional<Object> verifyAccessToken(final String accessToken) {
        final AccessTokenInfo accessTokenInfo = accessTokensByValue.get(accessToken);
        if (accessTokenInfo == null) {
            return Optional.empty();
        }
        return Optional.of(createVerifiedAccessToken(accessTokenInfo));
    }

    private void revokeTokenFamily(final String tokenFamilyId) {
        accessTokensByValue.entrySet().removeIf(e -> tokenFamilyId.equals(e.getValue().tokenFamilyId));
        refreshTokens.entrySet().removeIf(e -> tokenFamilyId.equals(e.getValue().tokenFamilyId));
    }

    private Object issueTokenPairInternal(final String identityName, final String requestedFamilyId) {
        final long sequence = tokenCounter.incrementAndGet();
        final String tokenId = "t" + sequence;
        final String familyId = requestedFamilyId != null ? requestedFamilyId : "f" + sequence;
        final String accessToken = "hdr." + tokenId + ".sig";
        final String refreshToken = "refresh-" + tokenId;
        final long now = System.currentTimeMillis();
        final long accessTokenExpirationDate = now + 60_000L;
        final long refreshTokenExpirationDate = now + 120_000L;

        accessTokensByValue.put(accessToken, new AccessTokenInfo(identityName, tokenId, familyId, accessTokenExpirationDate));
        refreshTokens.put(refreshToken, new TokenPairInfo(accessToken, familyId));

        return createTokenPair(accessToken, refreshToken, accessTokenExpirationDate, refreshTokenExpirationDate, familyId);
    }

    private Object createTokenPair(final String accessToken, final String refreshToken, final long accessTokenExpirationDate,
            final long refreshTokenExpirationDate, final String tokenFamilyId) {
        try {
            return tokenPairClass.getConstructor(String.class, String.class, long.class, long.class, String.class)
                    .newInstance(accessToken, refreshToken, accessTokenExpirationDate, refreshTokenExpirationDate,
                            tokenFamilyId);
        } catch (final Exception e) {
            throw new IllegalStateException("Cannot instantiate TokenPair", e);
        }
    }

    private Object createVerifiedAccessToken(final AccessTokenInfo accessTokenInfo) {
        try {
            return verifiedAccessTokenClass.getConstructor(String.class, String.class, String.class, long.class)
                    .newInstance(accessTokenInfo.identityName, accessTokenInfo.tokenId, accessTokenInfo.tokenFamilyId,
                            accessTokenInfo.expirationDate);
        } catch (final Exception e) {
            throw new IllegalStateException("Cannot instantiate VerifiedAccessToken", e);
        }
    }
    ...
    private static final class AccessTokenInfo {
        ...
        private AccessTokenInfo(final String identityName, final String tokenId, final String tokenFamilyId,
    ...
    private static final class TokenPairInfo {
        private final String accessToken;
        private final String tokenFamilyId;

        private TokenPairInfo(final String accessToken, final String tokenFamilyId) {
    ...
    public void cleanUp() {
        ...
        if (customizedConfig) {
            ...
            defaultConfig.put("auth.jwt.enabled", true);
            defaultConfig.put("session.management.enabled", true);
