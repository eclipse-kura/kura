...
public class ValidateOpenApiTest {
    ...
    private void thenOperationsHaveExplicitContracts() {
        this.document.path("paths").forEach(path -> path.forEach(operation -> {
            assertFalse(operation.path("summary").asText().isBlank());
            assertFalse(operation.path("description").asText().isBlank());
            assertFalse(operation.path("responses").has("default"));
            assertTrue(operation.path("responses").has("200") || operation.path("responses").has("204"));
