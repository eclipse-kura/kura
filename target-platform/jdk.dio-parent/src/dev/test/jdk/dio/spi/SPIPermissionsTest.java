 ...
 *
 * @test
 * @sources SPIPermissionsTest.java
 * @executeClass dio.spi.SPIPermissionsTest
 *
 * @title SPI permission verification
 *
 * @author stanislav.smirnov@oracle.com
 */
public class SPIPermissionsTest extends SPITestBase implements Test {
    ...
    private Status testPermissions() {
        ...
        while(devicesList.hasNext()){
            ...
            try {
                spiDevice = spiConfig.open();
                spiDevice.close();
                result = printFailedStatus("SPI device was opened without required permissions");
