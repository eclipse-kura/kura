...
# This script is used to build the KURA Launcher executables used to start Kura on Windows based systems. It relies on
...
if [ -x "$(command -v mingw32-make)" ] || [ -x "$(command -v make)" ]; then
	...
	echo "  WARNING: No viable 'make' command installed. KURA Windows Launcher will not be rebuilt"
fi
...
# pre built binaries in the Release directory to be used in the final installer. This should mean that you get a working launcher
...
if [ "$1" == "build" ]; then
		...
		if [ -x "$(command -v i686-w64-mingw32-gcc)" ] && [ -n "$MAKECMD" ]; then
			...
			echo "  WARNING: No viable make or 32 bit 'mingw' compiler installed. 32 bit KURA Windows Launcher will not be rebuilt"
		fi
		...
		if [ -x "$(command -v x86_64-w64-mingw32-gcc)" ] && [ -n "$MAKECMD" ]; then
			...
			echo "  WARNING: No viable make or 64 bit 'mingw' compiler installed. 64 bit KURA Windows Launcher will not be rebuilt"
		fi
fi
